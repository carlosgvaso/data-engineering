...
public class Hw7Jdbc {
	...
	private static final int tableSize = 50000;
	 ...
	 */
	private Hw7Jdbc(String dataGen, String physicalOrg, String indexCreation, String host, String user, String password) {
		 ...
		 */
		if (this.dg == Hw7Jdbc.DataGenerator.dgII) {
			System.out.print("Generating randomly ordered array for theKey...");
			
			this.dgIIPriKey = new ArrayList<>(Hw7Jdbc.tableSize);
			for (int i=1; i<=Hw7Jdbc.tableSize; i++){ 
		...
		for (int i=0; i<this.queryVal.length; i++) {
			...
			do {
				this.queryVal[i] = this.getRandomInt(Hw7Jdbc.randIntMin, Hw7Jdbc.randIntMax);
	...
	private void loadTableDGI() throws SQLException, SQLTimeoutException, NullPointerException {
		...
		for (int i=0; i<Hw7Jdbc.tableSize; i++) {
			...
			ps.setInt(2, this.getRandomInt(Hw7Jdbc.randIntMin, Hw7Jdbc.randIntMax));	// columnA ia a random int [1 - 50000]
			ps.setInt(3, this.getRandomInt(Hw7Jdbc.randIntMin, Hw7Jdbc.randIntMax));	// columnB is a random int [1 - 50000]
			ps.setString(4, this.getRandomStr(this.getRandomInt(Hw7Jdbc.randStrMin, Hw7Jdbc.randStrMax)));	// filler is a random string of length [0 - 247]
			ps.addBatch();
	...
	public static void main(String[] args) {
		...
		Hw7Jdbc hw6 = new Hw7Jdbc(args[0].toString(), args[1].toString(), args[2].toString(), args[3].toString(), args[4].toString(), args[5].toString());
		
		// Check we got good DG and PO
		boolean err = false;
		if (hw6.dg == Hw7Jdbc.DataGenerator.dgU) {
			System.out.println("ERROR: unknown data generator.");
			err = true;
		}
		if (hw6.po == Hw7Jdbc.PhysicalOrganization.poU) {
			System.out.println("ERROR: unknown physical organization.");
			err = true;
		}
		if (err) {
